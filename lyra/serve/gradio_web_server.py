...
# Helper functions for model name display mapping

def model_display_name(model_name):
    return model_name.replace("Lyra_", "Zen_")

def get_real_model_name(display_name):
    return display_name.replace("Zen_", "Lyra_")
...
# Regex pattern to redact large base64 audio
_BASE64_PATTERN = re.compile(r'data:audio/wav;base64,[A-Za-z0-9+/=]+')

def custom_showwarning(message, category, filename, lineno, file=None, line=None):
    message_str = str(message)
    if "audio/wav;base64" in message_str:
        message_str = _BASE64_PATTERN.sub('[base64_audio_redacted]', message_str)
    formatted = warnings.formatwarning(message_str, category, filename, lineno, line)
    (file or sys.stderr).write(formatted)

# Override default warning handler
warnings.showwarning = custom_showwarning
# Suppress Gradio info/debug logs
logging.getLogger("gradio").setLevel(logging.WARNING)

# Filter out any log message containing 'audio/wav;base64'
class NoLargeDataFilter(logging.Filter):
    def filter(self, record):
        return 'audio/wav;base64' not in str(record.getMessage())

logging.getLogger("gradio").addFilter(NoLargeDataFilter())
...
def load_demo(url_params, request: gr.Request):
    ...
    if "model" in url_params:
        ...
        if model in models:
            dropdown_update = gr.Dropdown(value=model_display_name(model), visible=True)
...
def load_demo_refresh_model_list(request: gr.Request):
    ...
    display_models = [model_display_name(m) for m in models]
    state = default_conversation.copy()
    dropdown_update = gr.Dropdown(
        choices=display_models,
        value=model_display_name(models[0]) if len(models) > 0 else ""
    )
    return state, dropdown_update
...
def add_text(state, text, speech, image, video, image_process_mode, instruct_mode, request: gr.Request):
    # print(state)  # Commented out to avoid printing binary audio data
    ...
    # print("text", text)
    # print("image", image)
    # print("speech", "Audio file provided" if speech else None)  # Avoid printing binary data
    # print("video", video)
...
def http_bot(state, model_selector, temperature, top_p, max_new_tokens, request: gr.Request):
    ...
    model_name = get_real_model_name(model_selector)
    ...
    # print(cur_prompt, len(state_tmp.messages))  # Commented out to avoid printing audio data
    ...
    # print(prompt)  # Commented out to avoid printing binary audio data
...
title_markdown = (f"""
# {lyra_svg_icon}&nbsp;Zen: An Efficient and Speech-Centric Framework for Omni-Cognition
""")
...
def build_demo(embed_mode, vocoder, cur_dir=None, concurrency_count=10):
    ...
    with gr.Blocks(title="Zen", theme='ParityError/Interstellar', css=block_css) as demo:
        state = gr.State()
        ...
        with gr.Row():
            with gr.Column(scale=3):
                ...
                imagebox = gr.Image(type="filepath", label='Upload image', height=300, visible=False)
                videobox = gr.Video(label='Upload video', height=300, visible=False)
            ...
            with gr.Column(scale=7):
                ...
                with gr.Column(scale=20):
                    audio_output_box = gr.Audio(label="Zen Speech Output", elem_classes="custom-audio")
                ...
                with gr.Row():
                    ...
                    with gr.Column():
                        ...
                        ], inputs=[imagebox, speech, instruct_mode], visible=False)
                        gr.Examples(examples=[
                            [f"examples/pred_round1.wav", "Record the spoken words as text.", 'text'],
                        ], inputs=[speech, textbox, instruct_mode], visible=False)
                    with gr.Column():
                        gr.Examples(examples=[
                            [f"examples/movement.mov", 'speech'],
                        ], inputs=[videobox, instruct_mode], visible=False)
                        gr.Examples(examples=[
                            [f"examples/Hi.mp3", 'speech'],
                        ], inputs=[speech, instruct_mode], visible=False)

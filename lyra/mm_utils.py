...
from lyra.constants import IMAGE_TOKEN_INDEX, SPEECH_TOKEN_INDEX, DEFAULT_IMAGE_TOKEN, DEFAULT_SPEECH_TOKEN, IGNORE_INDEX
import numpy as np
...
def tokenizer_image_speech_token(prompt, tokenizer, return_tensors=None):
    ...
    prompt_chunks = [tokenizer(chunk).input_ids 
                     for sublist in list(map(lambda part: part.split(DEFAULT_SPEECH_TOKEN), prompt.split(DEFAULT_IMAGE_TOKEN)))
                     for chunk in sublist]
        
    input_ids = []
    offset = 0
    if len(prompt_chunks) > 0 and len(prompt_chunks[0]) > 0 and prompt_chunks[0][0] == tokenizer.bos_token_id:
        # if has bos, then ignore it in concat
        offset = 1
        input_ids.append(prompt_chunks[0][0])

    # support we only have one image and one speech in current version, and IMAGE_TOKEN_INDEX is prior to SPEECH_TOKEN_INDEX
    for x in [ele 
              for sublist in zip(prompt_chunks, [[IMAGE_TOKEN_INDEX] * (offset + 1), [SPEECH_TOKEN_INDEX] * (offset + 1), [IGNORE_INDEX] * (offset + 1)]) 
